...
export const apiGetCidades = async (idCidade: string) => {
  const response = await http.get(ROTA.CIDADE.LISTAR)
  return response;
};


export const apiGetCidade = async (idCidade: string, model: Cidade) => {
...
export const apiDeleteCidade = async (idCidade: string, cidade: Cidade) => {
