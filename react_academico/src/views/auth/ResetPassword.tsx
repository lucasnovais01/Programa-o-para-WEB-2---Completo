...
export default function ResetPassword() {
  ...
  useEffect(() => {
    if (!token) {
      alert('Token inválido ou expirado. Redirecionando...');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setSuccess(true);
        setTimeout(() => navigate(ROTA.AUTH.LOGIN), 2500);
      } else {
        setErrors({ geral: data.mensagem || 'Erro ao redefinir senha' });
      }
    } catch (err) {
  ...
  return (
    <div className="display">
      <div className="card animated fadeInDown">
        ...
        {errors.geral && <div className="alert alert-danger">{errors.geral}</div>}
