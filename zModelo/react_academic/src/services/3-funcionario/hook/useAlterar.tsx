...
export const useAlterar = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const [resFuncionario, resFuncoes] = await Promise.all([
          apiGetFuncionario(Number(idUsuario)),
          apiGetFuncoes(1, 100),
        ]);

        if (resFuncionario?.data?.dados) {
          setModel(resFuncionario.data.dados as Funcionario);
        }
  ...
  const handleChangeField = (
    ...
    setModel((prev: Funcionario | null) => ({
      ...(prev ?? {}),
      [name]: value,
    } as Funcionario));
  ...
  const validarFormulario = (): boolean => {
    ...
    if (!model) {
      alert("Dados do funcionário não estão carregados");
      return false;
    }
  ...
  const onSubmitForm = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await apiPutFuncionario(Number(idUsuario), funcionarioToSend);
